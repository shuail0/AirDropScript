...
const SpaceSwap = require('../protocol/scroll/dex/spaceswap/spaceswap.js');
...
module.exports = async (params) => {
    ...
    // 将ETH兑换成USDC
    let tx = await spaceswap.swapEthToToken(wallet, min=ethers.BigNumber.from(0), wETH.address, usdc.address, amount);    
    ...
    await checkApprove(wallet, usdc.address, spaceswap.routerAddr, usdcBalance);
    

    tx = await spaceswap.swapTokenToEth(wallet, usdcBalance, min=ethers.BigNumber.from(0), usdc.address, wETH.address);
