...
const veSyncSwap = require('../protocol/zksync/dex/veSync/veSync.js');
...
module.exports = async (params) => {
    ...
    const vesyncswap = new veSyncSwap();
    ...
    // // 将ETH兑换成USDC
    // let tx = await vesyncswap.swapEthToToken(wallet, wETH.address, usdc.address, amount, min=ethers.BigNumber.from(0));    
    ...
    await checkApprove(wallet, usdc.address, vesyncswap.routerAddr, usdcBalance);

    tx = await vesyncswap.swapTokenToEth(wallet, usdc.address, wETH.address, usdcBalance, min=ethers.BigNumber.from(0));
