...
const Skydrome = require('../protocol/scroll/dex/skydrome/skydrome.js');
...
module.exports = async (params) => {
    ...
    // 将ETH兑换成USDC
    let tx = await skydrome.swapEthToToken(wallet, min=ethers.BigNumber.from(0), wETH.address, usdc.address, amount);    
    ...
    await checkApprove(wallet, usdc.address, skydrome.stableRouterAddr, usdcBalance);
    

    tx = await skydrome.swapTokenToEth(wallet, usdcBalance, min=ethers.BigNumber.from(0), usdc.address, wETH.address);
